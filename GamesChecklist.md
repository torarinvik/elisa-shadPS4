...
| [x] | The Witch and the Hundred Knight 2 | CUSA10135 | Tested. Boots, initializes MoltenVK/Vulkan, and renders nonblack frames; no black-screen watchdog or GPU wait timeout fired. Then crashes in `PhyreEngineWorkerThread0` with a null write: `Unhandled access violation at code address 0x809d77b7d: Write to address 0x0`. Repeated `sceNgs2...` audio errors appeared before the crash, so this looks like a guest/engine/audio-library compatibility issue rather than the UFC render-freeze bucket. |
...
| [x] | Race With Ryan Road Trip Deluxe Edition | CUSA23279 | Tested. Boots through loader/Vulkan init, then blocks on the known fixed-address mapping bucket: `sceKernelMapNamedDirectMemory` requests `0x4000000000` for `0x8000000` bytes, overlapping the macOS x86_64-on-Apple-Silicon GPU-reserved address hole `0x1000000000-0x6fffffffff`. It then crashes with `Unhandled access violation at code address 0x80c0aba94: Write to address 0x0`. |
