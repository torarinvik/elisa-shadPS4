...
| [x] | YAKUZA 6: The Song of Life | CUSA09660 | Retested after being added locally. Boots far enough to load many Yakuza assets/audio streams, then crashes in the shader/SPIR-V emitter: `EmitSetAttribute: Unreachable code! Write attribute StencilRef`. Same blocker as YAKUZA KIWAMI 2. |
...
| [x] | YAKUZA KIWAMI 2 | CUSA10634 | Retested after being added locally. Boots into asset/audio/render initialization and loads `.awb` audio, then crashes in the shader/SPIR-V emitter with `EmitSetAttribute: Unreachable code! Write attribute StencilRef`. Same blocker as YAKUZA 6. |
...
| [x] | New Super Lucky's Tale | CUSA20302 | Retested after the recent rendering/I/O hardening. The old strict render-validation stop on unsupported `B4G4R4A4UnormPack16` no longer blocks progress: it enters the game and audio/subtitles/UI proceed. Current state: partial rendering with a large solid green frame and some tiny visible fragments, so this has moved from launch/render abort to a gameplay-path render-format/texture decode bug. |
